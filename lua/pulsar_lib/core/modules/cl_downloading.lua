...
function PulsarLib.Modules.GetMetadata(callback)
    ...
    local metadataTable = util.JSONToTable(metadata)
    callback(true, metadataTable)
...
function PulsarLib.Modules.DownloadModuleMetaData(name, callback)
    ...
    PulsarLib.Modules.GetMetadata(function(success, metadata)
        if not success or not metadata or not metadata[name] then
        ...
        if (lastMetaTimes[name] and lastMetaTimes[name] > os.time() - (60 * 5)) and file.Exists("pulsarlib/modules/" .. name .. "/metadata.json", "DATA") then -- 5 mins
            PulsarLib.Logging:Debug("Using cached module metadata for '", logger:Highlight(name),
                "' as it is less than 5 minutes old")
            callback(true)
            return
        end
        ...
        if not file.Exists("pulsarlib/modules/" .. name .. "/metadata.json", "DATA") then
            http.Fetch(moduleDataURL, function(body, size, headers, code)
                if code == 200 then
                    ...
                    PulsarLib.Logging:Error("Failed to download module metadata for '", logger:Highlight(name), "': '",
                        logger:Highlight(code), "'")
                    callback(false)
                end
            end)
        else
